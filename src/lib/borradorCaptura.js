 ...
 *
 * Esto guarda el stand en curso en la base local del teléfono cada vez que
 * cambia (con una espera corta para no escribir en cada tecla), y al volver a
 * entrar ofrece retomarlo. Se borra cuando el stand se guarda de verdad.
 *
 * Vive en la tabla `settings` (clave propia) para no cambiar el esquema de la
 * base. La pieza 4.3 (cada foto se guarda al instante) lo vuelve innecesario.
 */
...
export function tieneContenido(b) {
  if (!b) return false;
  return (b.items?.length || 0) > 0 || !!b.cardPhoto || !!(b.supplierName && b.supplierName.trim());
}
